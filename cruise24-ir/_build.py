...
# though this generator does not write them. Discovered by crawling the live
# site, not assumed — /search.html is deliberately absent because it 301s to
# book.cruise24.ir, which carries its own noindex.
EXISTING = ["index.html", "blog.html", "cruiseletter.html", "events.html",
            "cruise-lines.html"]
...
<!--
  ...
  Regenerated by _build.py. The previous sitemap listed four URLs while nine
  pages existed: cruise-lines, msc, aroya, celestyal and explora were all live
  and none of them was declared, so none could be found.
...
def main(argv=None):
    ...
    print(f"\n  sitemap.xml  {len(EXISTING) + len(pages)} URLs "
