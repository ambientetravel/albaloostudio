    ...
    try:
        ...
        rp = requests.post(
            ...
            json={"title": f"دریانامه draft: {article['title']}"[:72],
                  ...
                  "body": (f"Drafted by Agent 2 from the Search Console gap "
                           f"`{brief.opportunity.primary_keyword}`.\n\n"
                           f"Adds one object to `{path}`.\n\n"
                           f"**Run `python3 _articles.py && python3 _sitemap.py` "
                           f"before merging**, and commit what they write. The "
                           f"article needs a real page at `daryanameh/{slug}.html`; "
                           f"`article.html?a={slug}` is retired and now redirects "
                           f"there. All four earlier pieces went 90 days without a "
                           f"single impression because they only ever existed at "
                           f"the `?a=` shell, which canonicalises every article to "
                           f"one URL and ships no body in the HTML. CI fails this "
                           f"PR until the page and the sitemap are regenerated.\n\n"
