...
seed = {
    ...
    'side_bias_NOT_ESTABLISHED': (
        'A 54% win rate for side A was reported and it does NOT replicate. With '
        'identical decks on both sides the figure moves to 44% / 53% / 64% across '
        'the three rival styles — the direction is not even consistent, so the '
        'original reading was one configuration mistaken for a finding. Two '
        'confounds were in it: each side drew its deck from a different seed, and '
        'the two sides legitimately receive different offers. A real measurement '
        'needs both controlled and far more than 120 matches per cell; at n=120 the '
        'standard error alone is about 4.5 points. Do not act on the 54% figure and '
        'do not ask an analyst to explain it.'
    ),
    'decided_and_closed': [
