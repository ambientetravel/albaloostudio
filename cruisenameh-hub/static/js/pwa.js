/* کروزنامه — pwa.js: service worker registration, install prompt, save-for-offline, saved list. */
(function () {
  if (!('serviceWorker' in navigator)) return;
  navigator.serviceWorker.register('/sw.js').catch(() => {});
  let deferred; addEventListener('beforeinstallprompt', e => { e.preventDefault(); deferred = e; if (!localStorage.getItem('dn-install-dismissed')) window.toast && toast('کروزنامه را مثل یک اپ نصب کنید؛ راهنمای بندرها بدون اینترنت.', { label: 'نصب', fn: () => deferred.prompt() }); try { localStorage.setItem('dn-install-dismissed', '1'); } catch (_) {} });
